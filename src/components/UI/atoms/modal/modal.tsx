      ...
      <ModalStyle {...props} onMouseDown={modal_close}>
        {props.test ? (
          <div ref={modal_ref} className="board_div">
            <div className="board_title">{props.board.title}</div>
            <div className="board_info">
              <div className="board_info_delete">
                <Button>삭제</Button>
              </div>
              작성자 : {props.board.name}
              <br />
              작성일 :{' '}
              {moment(props.board.share_date).format('YYYY년MM월DD일 HH:mm')}
            </div>
            <div className="board_content_div">
              <div className="board_content">
                ...
                <br />
                <br />총 시간 :{' '}
                {cal(props.board.start_time, props.board.end_time)} 분
              </div>
            </div>
            <div className="board_reply_input">
              <input
                type="text"
                value={props.reply}
                onChange={(e) => props.set_reply(e.target.value)}
              ></input>
              <div className="board_reply_button">
                <Button onClick={props.modal_reply}>등록</Button>
              </div>
            </div>
              ...
              ? props.reply_data.map((data) => {
                  return (
                    <div className="board_reply_div">
                      <div className="board_reply_name">{data.name}</div>
                      <div className="board_reply_date">
                        {moment(data.reply_date).format('YYYY년MM월DD일 HH:mm')}
                      </div>
                      <div className="board_reply_comment">{data.reply}</div>
                    </div>
                  );
                })
              : ''}
